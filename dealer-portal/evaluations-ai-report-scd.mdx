...
description: "Live GET /api/v1/bikes/:id/ai-report/ reads unmanaged bikes_bikeevaluationresult via parse_ai_report. evaluations.BikeEvaluation Type-2 SCD and BikeEvaluationSerializer are unused by that endpoint. BikeAssessmentViewSet is a separate ModelViewSet on Bike."
---
...
- `final_price_vnd` equals `pricing_estimation.final_buyback_price_vnd` cast to int, else null.
- `price_range` is an object with min and max from `pricing_estimation.market_anchor`, or null if both min and max are missing.
- `justification` equals `pricing_estimation.justification`.
- `evaluated_at` equals `result.updated_at.isoformat()`. Django `auto_now=True` on the unmanaged model does not write unless dealer calls save. TMA must populate `updated_at` on the real table.
...
- Dedup key is component\_name plus text. Empty text is dropped.
- Issue entries may be string or dict; dict prefers keys `issue`, `aspect`, `description`, `text`, `note`.
- JSON path for components: section.technical\_data.components\_evaluation array.
...
`condition_score`: average of `component_condition_score` across both sections. If **every** score is in range -1.0 to 1.0 inclusive, map with int of (avg plus 1) divided by 2 times 100, then clamp 0-100. Otherwise int(avg) clamped 0-100. No scores yields null.
...
There is **no** 200 with `has_report: false`. FE comment that 404 or has\_report false are handled the same is defensive; the backend never returns false.
...
Docstring mentions `update_assessment`; **that action is not implemented**. SPA catalog does not call this ViewSet. Treat destroy-on-Bike as a live authenticated capability.
...
`BikeAssessment` severity choices: `good`, `warning`, `error`. Parser item `status` uses the same three strings, but assessments are **manual rows**, not derived from AI JSON.
...
`BikeContent.tsx` walks combo member bike\_id list then bike\_ids. First 200 with `has_report` wins. Catch-all on any throw (including 404) tries the next UUID. If none succeed, `aiReport` is null and the section is omitted.
...
1. Combo with multiple bike ids: first bike that has a result hides later bikes reports.
2. Empty JSON blobs still 200 `has_report` true with empty items and null score/price. UI shows header with no rows.
3. Mixed score scales: if one component is 0.8 and another is 75, the -1 to 1 branch is skipped and int(avg) can be a nonsense mid-range.
4. Missing Bike vs missing result are different 404 shapes (DRF default vs Vietnamese detail).
...
8. ai-report is v1-only. A client using legacy `/api/` prefix will not find this route.