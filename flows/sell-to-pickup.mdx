...
Money path: rider lists a bike → mobile evaluate / TIPP → OPS enters **FMP only** (blind to TIPP on input) → internal snapshots listing \+ TIPP → mobile pulls operator-offer over HMAC → rider picks `listing` or `instant_purchase` → pickup → sale complete. Unit tests cover pieces. No single automated test owns the full cross-repo path yet.
