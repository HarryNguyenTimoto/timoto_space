...
description: "POST /api/webhooks/tma/new-submission/ verifies X-Timoto-Signature sha256 HMAC with TMA_WEBHOOK_SECRET, requires bike_id, get_or_create SlaTracker, creates InternalNotification NEW_SUBMISSION, best-effort notify_new_submission FCM, always returns ok true when auth and JSON valid."
---
...
| Method | Path | Auth | Body | | --- | --- | --- | | POST | `/api/webhooks/tma/new-submission/` | AllowAny \+ HMAC header | JSON with required `bike_id` |
